...
||
ID | ID number assigned to the simulation | Generated by admin
...
#### BATCHID
Give an identifier for a series of related simulations e.g. NVT simulations with the same composition but different volume to compute a surface pressure-area per lipid isoterm.
...
#### ID
A numerical identifier of the simulation, used for access systems from the API and is displayed in the GUI. It must be defined by the admin.
